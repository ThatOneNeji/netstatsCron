...
function MessageBroker() {
    let connection;
    let publishChannelWrapper;
    //    var receiveChannelWrapper;
    const receiveChannelObject = {};
    ...
    this.createAdditionalConsumerQueues = function(queueNames) {
    ...
    function createConsumerQueue(queueName) {
        // function createConsumerQueue(queueName) {
        const instanceName = cleanQueueName(queueName);
        ...
        receiveChannelObject[instanceName] = connection.createChannel({
            setup: function(channel) {
                return Promise.all([
                    channel.assertQueue(queueName, createChannelOptions),
                    channel.prefetch(1),
    ...
    function initialisePublishQueue(queueName) {
        if (queueName) {
            publishChannelWrapper = connection.createChannel({
                json: true,
                setup: function(channel) {
                    return channel.assertQueue(queueName, createChannelOptions);
                    // return channel.publish(queueName, createChannelOptions);
                }
            });
    ...
    this.init = function(options = {}) {
        ...
        connection.on('connect', function(connectionn) {
            ...
            if (receiveQueue) {
                ...
                createConsumerQueue(receiveQueue);
            }
            // console.log('3');
        });
    ...
    function postMessageToQueue(msg) {
        // Logging.error(publishChannelWrapper);
        publishChannelWrapper.sendToQueue(msg.queuename, msg.data, publishChannelOptions)
            // publishChannelWrapper.publish('', msg.queuename, msg.data, publishChannelOptions)
            .then(function(ok) {
                /*            if (!ok) {
